...
class RandomSplitOperator(DataProcessingOperator):
    ...
    def handle(self, dataframe_list, spark):
        # 1.参数的获取
        left_weight = float_convert(self.conf.get("left_weight"), "left_weight", self.op_id)
        right_weight = float_convert(self.conf.get("right_weight"), "right_weight", self.op_id)
