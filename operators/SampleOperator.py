...
class SampleOperator(DataProcessingOperator):
    ...
    def handle(self, dataframe_list, spark):
        # 1、参数获取
        with_replacement = bool_convert(self.conf.get("with_replacement", False), "with_replacement", self.op_id)
        fraction = float_convert(self.conf.get("fraction"), "fraction", self.op_id)
        ...
        if seed is None:
            ...
            seed = float_convert(seed, "seed", self.op_id)
