...
class LabelFeatureToLibsvmOperator(DataProcessingOperator):
    ...
    def handle(self, dataframe_list, spark):
        ...
        is_drop_input = self.conf.get("is_drop_input")
        ...
        if is_drop_input is None:
            raise ParameterException("[arthur_error] the conf parameter:is_drop_input is error"+str(self.op_id))
        else :
            is_drop_input = bool_convert(is_drop_input,"is_drop_input", self.op_id)

        # 3、映射转化
        rdd = df.rdd.map(lambda row: self.map_function(row, label, column_name, output, is_drop_input))
        df = spark.createDataFrame(rdd)
        return [df]

    def map_function(self, row, label, col_name, output_col, is_drop_input):
        '''
        ...
        try:
            ...
            raise ParameterException("[arthur_error]the input label type must could convert to a float, op_id:" + str(self.op_id))
        ...
        if "pyspark.ml.linalg.SparseVector" in type_feature or "pyspark.ml.linalg.DenseVector" in type_feature:
            ...
            raise ParameterException(
                "[arthur_error] the input vector type is not a support type:" + type_feature + ", opid" + str(self.op_id))
        ...
        try:
            ...
            dict = row.asDict(True)

            if is_drop_input:
                dict.pop("label")
                dict.pop("col_name")
            else :
                dict[output_col] = str_libsvm
