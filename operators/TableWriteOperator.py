...
"""  
    ...
    conf_write = {
                    ...
                    "partition_by": String[col_name]
                    "is_partition_by_date":bool
                  };
    3、结果表：
"""
...
class TableWriteOperator(DataProcessingOperator):
    ...
    def handle(self, dataframe_list, spark):
        ...
        try:
            # 按照已有的列做分区
            if partition_by:
                partition_by = str_convert_strlist(partition_by, "partition_by", self.op_id)
                check_cols(partition_by, df.columns, self.op_id)
