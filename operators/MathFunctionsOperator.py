...
class MathFunctionsOperator(DataProcessingOperator):
    ...
    def handle(self, dataframe_list, spark):
        ...
        for conf in scaler_conf:
            ...
            is_replace.append(bool_convert(conf[2], "is_replace", self.op_id))
            new_col_name.append(conf[3])

        # 2、参数的检查
        check_strlist_parameter(col_names, "col_names", self.op_id)
        check_strlist_parameter(scale_method, "scale_method", self.op_id)
        check_strlist_parameter(new_col_name, "new_col_name", self.op_id)
