    ...
    def handle(self, dataframe_list, spark):
        ...
        try:
            ...
            for conf in smooth_conf:
                ...
                if col_type not in self.support_type and col_type not in "decimal":
                    raise ParameterException(
                        "[arthur_error] the input colums type must be a number type, but now is a :" + str(col_type))

                min_thresh = float_convert(conf[1], "min_thresh", self.op_id)
                max_thresh = float_convert(conf[2], "max_thresh", self.op_id)
