            ...
            for i, conf in enumerate(normalize_scaler_conf):
                if len(conf) < 4:
                    raise ParameterException(
                        "[arthur_error] the lengths of parameter must more than 5:" + str(conf) + "opid:" + str(self.op_id))
                # 参数解析和校验
                input_col = conf[0]
                output_col = conf[1]
                min = conf[2]
                ...
                if not min:
                    ...
                    min = float_convert(min, "min", self.op_id)

                if not max:
                    max = 1
                else:
                    max = float_convert(max, "max", self.op_id)

                is_drop_input = bool_convert(conf[4], "is_drop_input", self.op_id)
                ...
                if type == "vector":
                ...
                elif type in self.support_type and type in "decimal":
                    ...
                    raise ParameterException(
                        "[arthur_error] input col must be bigint/double/vector,does not support: " + type + "opid:" + str(self.op_id))
