...
class ApproxQuantileOperator(DataProcessingOperator):
    ...
    def handle(self, dataframe_list, spark):
        ...
        # 2、参数转换与检查
        input_cols = str_convert_strlist(input_cols, "input_cols", self.op_id)
        probabilities = str_convert_floatlist(probabilitie_str, "probabilities", self.op_id)
        relative_error = float_convert(relative_error, "relative_error",  self.op_id)
    ...
    def check_input_cols_type(self, input_cols, df_col_schema):
        if not input_cols or not df_col_schema:
            raise ParameterException("[arthur_error] the parameter is error, op_id:" + str(self.op_id))
        for col in input_cols:
            if df_col_schema[col] not in self.support_type and df_col_schema[col] not in "decimal":
                raise InputColumnTypeException(
                    "[arthur_error] the input columns type is not support, must a numeric type, opid:" + str(self.op_id))
